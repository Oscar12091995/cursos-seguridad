        ...
        <div class="px-6 py-4">
           @if ($goal->id == $item->id)
            <form wire:submit.prevent="update">
                <input wire:model="goal.name" class="form-control w-full">
                @error('goal.name')
    ...
    <article class="bg-white shadow-lg rounded overflow-hidden">
        <div class="px-6 py-4 ">
            <form class="" wire:submit.prevent="store">
                <input wire:model="name" class=" border border-green-500 text-sm rounded-lg  focus:border-green-500 block w-full p-2.5 " placeholder="Escribe la meta del curso...">
                @error('name')
